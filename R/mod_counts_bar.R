...
mod_counts_bar_server <- function(id, vals) {
  moduleServer(id, function(input, output, session) {
    ...
    logr_msg("Initializing horizontal bar plot module", level = "INFO")
    
    output$hbar_graph <- renderPlot({
      logr_msg("Attempting to render horizontal bar plot", level = "TRACE")
      ...
      tryCatch({
        ...
        logr_msg("Horizontal bar plot module initialized", level = "INFO")
      })
    }