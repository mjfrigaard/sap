...
`scatter_plot()` is flexible because it uses data masking (i.e., `.data[[x_var]]`), so we could potentially use it in any module with a scatter plot output. 

In `18_html`, the `mod_point_vars` module collects user inputs that are rendered with `plotly` functions directly in the `mod_point` module server function: 
