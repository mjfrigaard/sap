...
This document contains the updated graphs in `sap` (beginning in the [`18_html branch`](https://github.com/mjfrigaard/sap/tree/18_html))
...
I've adapted the `mod_point_vars` module to collect the user inputs and render the scatter plot using `plotly` functions directly in the `mod_point` module:
...
2.  `plotly::marker` specifies the size (`vals()$size`) and opacity (`vals()$alpha`) of the points. The `type = 'scatter'` and `mode = 'markers'` arguments specify the scatter plot with 'points.'
...
The raincloud plot is a combination of layers from `geom_boxplot()`, `ggdist::stat_halfeye()`,  and `geom_point()`. The reactive values come from the `vals()` reactive object, which contains one numeric (`num_var`) and one character variable (`chr_var`), and the `alpha` and `size` aesthetics:

```{r}
#| label: raincloud_vars
#| eval: true
vals <- list('num_var' = name_case('audience_score'),
              'chr_var' = name_case('critics_rating'), 
              'alpha' = 0.75,
              'size' = 3)
vals
```
...
The movie awards plot uses a subset of the `movies` dataset generated from the `best_`, `director`, and `actor*` columns.

```{r}
dplyr::glimpse(movies)
```

The inputs are the type of award: `"picture/director"`, `"actor/actress"`. 

### Awards: picture/director

If the user selects `"picture/director"`, the function returns the `title`, `release_year` and `director` columns:

```{r}
awards <- get_award_recipients(movies, "picture/director", 1992)
dplyr::glimpse(awards)
```
...
ggp2_points <- ggplot2::ggplot(data = awards) + 
  ggplot2::aes(x = release_year, 
               y = title, 
               label = director_genre) + 
...
#| eval: true
library(ggrepel)
ggp2_repel <- ggp2_points + 
  ggrepel::geom_label_repel(
    mapping = aes(fill = director_genre),
          color = "#FFFFFF",
          max.iter = 1e5,
          max.time = 1,
          point.padding = 0.5,
...
### Awards: actor/actress

If the user selects `"actor/actress"`, the function returns the `title`,  `release_year`, and `star` columns:

```{r}
awards <- get_award_recipients(movies, "actor/actress", 1987)
dplyr::glimpse(awards)
```
