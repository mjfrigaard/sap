...
This vignette explains the downloadable PDF report in the `sap` R package/Shiny application. A folder tree of it's contents is below:
...
mod_report_download_server <- function(id, compare_values, count_values,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$download_report <- downloadHandler(
      filename = function() {
        paste0("movie-analysis-report-", format(Sys.time(), "%Y-%m-%d-%H%M%S"), ".pdf")
      },
      content = function(file) {
        params <- report_params()
        
        # show notification while the report is generating
        id <- showNotification(
          "Generating report...", 
          duration = NULL, 
          closeButton = FALSE,
        ...
        # create temp directory for report gen
        report_dir <- tempdir()
        rmd_path <- file.path(report_dir, "report.Rmd")
        
        # copy report template to temp directory
        file.copy(
        ...
        tryCatch({
          ...
          logr_msg(glue::glue("Error generating report: {e$message}"), level = "ERROR")
          showNotification(
